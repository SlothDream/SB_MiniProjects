...
SELECT DISTINCT CONCAT(Members.firstname, ' ', Members.surname) AS member, Facilities.name
FROM Members
JOIN Bookings ON Bookings.memid = Members.memid
JOIN Facilities ON Facilities.facid = Bookings.facid
WHERE Facilities.name LIKE 'Ten%'
ORDER BY member
...
FROM (SELECT CONCAT(Members.firstname,  ' ', Members.surname) AS member, Facilities.name AS facility, 
      (CASE WHEN Bookings.memid =0 THEN (Facilities.guestcost * Bookings.slots)
	...
	JOIN Bookings ON Bookings.memid = Members.memid
	JOIN Facilities ON Facilities.facid = Bookings.facid
	WHERE Bookings.starttime LIKE  '2012-09-14%') AS bookings
WHERE cost > 30
ORDER BY cost DESC
...
SELECT Facilities.name AS facility, SUM(Bookings.slots * 
